...
const Sidebar: React.FC<SidebarProps> = ({ active, onSelect }) => {
  const { user, credits, isLoggedIn, isLoading, setShowAuthModal, signOut } = useAuth();
        ...
        {NAV_ITEMS.map(({ tool, icon, label }) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-5 py-3 text-left transition-all group ${
              active === tool ? 'bg-blue-600/15 text-white border-r-2 border-blue-500' : 'text-gray-500 hover:text-gray-300'
            }`}
          >
            <i className={`fas ${icon} text-[11px] w-4 text-center ${active === tool ? 'text-blue-400' : 'text-gray-600'}`}></i>
      ...
      <div className="shrink-0 border-t border-white/5 p-4 space-y-3">
        {isLoading ? (
          <div className="animate-pulse flex items-center justify-center py-4">
             <span className="text-[8px] text-gray-600 uppercase tracking-widest">Verifying...</span>
          </div>
        ) : isLoggedIn ? (
          <>
            <div className="flex items-center justify-between cursor-pointer" onClick={() => onSelect(ToolType.PRICING)}>
              ...
              <div className={`flex items-center gap-1 px-2 py-0.5 rounded-lg text-[9px] font-black uppercase tracking-widest border ${
                creditsLow ? 'bg-red-500/10 text-red-400 border-red-500/20' : 'bg-purple-500/10 text-purple-400 border-purple-500/20'
              }`}>
                {credits !== null ? <span>{credits} CR</span> : <span>···</span>}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0">
                <div className="w-6 h-6 rounded-full bg-blue-600/20 flex items-center justify-center shrink-0">
                  <span className="text-[8px] font-black text-blue-400 uppercase">{user?.email?.[0] ?? '?'}</span>
                </div>
                <span className="text-[9px] text-gray-600 truncate">{user?.email}</span>
              </div>
              <button onClick={signOut} className="text-gray-700 hover:text-gray-400 ml-2">
          ...
          >
            ...
            <span className="text-[8px] text-purple-400/70 font-bold uppercase tracking-widest">Sign in to get started</span>
          </button>
        )}
      </div>
    </div>
